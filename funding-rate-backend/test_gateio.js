// test_gateio.js - Test Gate.io API
import 'dotenv/config';
import { gateioHandler } from './services/exchangeHandlers/gateioHandler.js';
import { hasCredentials, MODE } from './services/config.js';

// ====================================================================
// CÀI ĐẶT TEST
// ====================================================================
const TEST_SYMBOL = 'BTCUSDT'; // Symbol để test (sẽ được chuyển thành BTC_USDT)
const TEST_LEVERAGE = 10;      // Đòn bẩy để test
const TEST_QUANTITY = 1;       // Số lượng hợp đồng để test (Gate.io dùng số nguyên)
// ====================================================================

async function runGateioTests() {
  console.log('🚀 Bắt đầu test Gate.io API...');
  console.log('====================================\n');

  // Kiểm tra credentials
  if (!hasCredentials('gateio')) {
    console.error('❌ Vui lòng cung cấp GATEIO_API_KEY và GATEIO_SECRET_KEY trong file .env');
    return;
  }
  ...
  try {
    ...
    const price = await gateioHandler.getPrice(TEST_SYMBOL);
  ...
  try {
    ...
    const info = await gateioHandler.getSymbolInfo(TEST_SYMBOL);
  ...
  try {
    ...
    const pnlInfo = await gateioHandler.getPNL(TEST_SYMBOL);
  ...
  try {
    ...
    await gateioHandler.setMarginType(TEST_SYMBOL, 'ISOLATED');
  ...
  try {
    ...
    await gateioHandler.setLeverage(TEST_SYMBOL, TEST_LEVERAGE);
  ...
  try {
    ...
    const orderResult = await gateioHandler.placeOrder(TEST_SYMBOL, 'BUY', TEST_QUANTITY);
    ...
    const newPnlInfo = await gateioHandler.getPNL(TEST_SYMBOL);
    console.log('   ✅ PNL mới:');
  ...
  if (orderId) {
    try {
      ...
      const closeResult = await gateioHandler.closePosition(TEST_SYMBOL);

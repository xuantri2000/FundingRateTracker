...
async function runMexcTests() {
  ...
  try {
    ...
    const price = await mexcHandler.getPrice(TEST_SYMBOL);
  ...
  try {
    ...
    const info = await mexcHandler.getSymbolInfo(TEST_SYMBOL);
  ...
  try {
    ...
    const pnlInfo = await mexcHandler.getPNL(TEST_SYMBOL);
  ...
  try {
    ...
    await mexcHandler.setMarginType(TEST_SYMBOL, 'ISOLATED');
  ...
  try {
    ...
    await mexcHandler.setLeverage(TEST_SYMBOL, TEST_LEVERAGE);
  ...
  try {
    ...
    const orderResult = await mexcHandler.placeOrder(TEST_SYMBOL, 'BUY', TEST_QUANTITY);
    ...
    const newPnlInfo = await mexcHandler.getPNL(TEST_SYMBOL);
    console.log('   ✅ PNL mới:');
  ...
  if (orderId) {
    try {
      ...
      const closeResult = await mexcHandler.closePosition(TEST_SYMBOL);
